...
class WU_WooCommerce_Optimizer {
    ...
    public function admin_init() {
        ...
        $settings = array(
            ...
            'wu_woo_taiwan_address',
            'wu_woo_enable_island_shipping',
            'wu_woo_enable_711_shipping',
            'wu_woo_711_shipping_cost',
            'wu_woo_711_free_shipping_threshold',
            'wu_woo_enable_order_comments',
            'wu_woo_enable_einvoice'
        );
        ...
        add_settings_field(
            'wu_woo_taiwan_address',
            '台灣地址選單優化',
            array($this, 'taiwan_address_callback'),
            'wu_woocommerce_settings',
            'wu_woocommerce_section'
        );
        ...
        add_settings_field(
            'wu_woo_enable_island_shipping',
            '啟用台灣離島運送',
            array($this, 'enable_island_shipping_callback'),
            'wu_woocommerce_settings',
            'wu_woocommerce_section'
        );
        ...
        add_settings_field(
            'wu_woo_enable_711_shipping',
            '啟用 7-11 超商取貨',
            array($this, 'enable_711_shipping_callback'),
            'wu_woocommerce_settings',
            'wu_woocommerce_section'
        );
        ...
        add_settings_field(
            'wu_woo_enable_order_comments',
            '啟用訂單備註欄位',
            array($this, 'enable_order_comments_callback'),
            'wu_woocommerce_settings',
            'wu_woocommerce_section'
        );
        ...
        add_settings_field(
            'wu_woo_enable_einvoice',
            '啟用電子發票功能',
            array($this, 'enable_einvoice_callback'),
            'wu_woocommerce_settings',
            'wu_woocommerce_section'
        );
    }
...
jQuery(document).ready(function($) {
    ...
    public function admin_page() {
        ...
        if (isset($_POST['submit'])) {
            ...
            foreach (array('wu_woo_disable_notifications', 'wu_woo_show_sales_with_offset', 'wu_woo_taiwan_address', 'wu_woo_enable_island_shipping', 'wu_woo_enable_711_shipping', 'wu_woo_enable_order_comments', 'wu_woo_enable_einvoice') as $s) {
                update_option($s, isset($_POST[$s]) ? 1 : 0);
            }
            
            if (isset($_POST['wu_woo_711_shipping_cost'])) {
                update_option('wu_woo_711_shipping_cost', intval($_POST['wu_woo_711_shipping_cost']));
            }
            if (isset($_POST['wu_woo_711_free_shipping_threshold'])) {
        ...
        ?>
        <div class="wrap">
            <h1>WooCommerce 優化器</h1>
            <div class="card">
                <h2>功能狀態</h2>
                <ul>
                    <li>通知欄停用: <?php echo get_option('wu_woo_disable_notifications') ? '✓ 已啟用' : '✗ 未啟用'; ?></li>
                    <li>購買量顯示: <?php echo get_option('wu_woo_show_sales_with_offset') ? '✓ 已啟用' : '✗ 未啟用'; ?></li>
                    <li>台灣地址選單: <?php echo get_option('wu_woo_taiwan_address') ? '✓ 已啟用' : '✗ 未啟用'; ?></li>
                    <li>台灣離島運送: <?php echo get_option('wu_woo_enable_island_shipping') ? '✓ 已啟用' : '✗ 未啟用'; ?></li>
                    <li>7-11 超商取貨: <?php echo get_option('wu_woo_enable_711_shipping') ? '✓ 已啟用 (運費: NT$' . get_option('wu_woo_711_shipping_cost', 60) . ', 免運門檻: NT$' . get_option('wu_woo_711_free_shipping_threshold', 0) . ')' : '✗ 未啟用'; ?></li>
                    <li>訂單備註欄位: <?php echo get_option('wu_woo_enable_order_comments') ? '✓ 已啟用' : '✗ 未啟用'; ?></li>
                    <li>電子發票功能: <?php echo get_option('wu_woo_enable_einvoice') ? '✓ 已啟用' : '✗ 未啟用'; ?></li>
                </ul>
            </div>
            ...
            <div class="card">
                ...
                <h3>電子發票功能(v4.3 新增)</h3>
                <ul>
                    <li>支援三種發票類型:個人、公司、捐贈</li>
                    <li>個人發票:可選擇手機載具或紙本發票</li>
                    <li>公司發票:輸入統一編號自動查詢公司名稱(使用財政部開放資料API)</li>
                    <li>捐贈發票:可輸入3-7位數捐贈碼</li>
                    <li>發票資訊會儲存至訂單並在後台顯示</li>
                </ul>
                
                <h3>7-11 超商取貨功能</h3>
                <ul>
                    <li>啟用後會自動新增「7-11 超商取貨」運送方式</li>
                    <li>可自訂運費金額及免運門檻</li>
                    <li>收件者電話為必填欄位</li>
                    <li>選擇 7-11 取貨時自動勾選「寄送到其他收件地址」</li>
                    <li>自動記錄登入者的門市資訊</li>
                </ul>
                
                <h3>台灣地址選單</h3>
                <ul>
                    <li>縣市/鄉鎮市區/郵遞區號三級聯動</li>
                    <li>支援台灣本島19縣市及離島3縣</li>
                    <li>郵遞區號雙向同步</li>
                </ul>
            </div>
        </div>
