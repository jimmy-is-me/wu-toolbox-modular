...
class WUTM_Discord_Sale_Notifications {

    const OPTION_GROUP = 'wutm_discord_notifications';
    const PAGE_SLUG = 'wu-discord-notifications';
    ...
    public function add_settings_page() {
        $capability = apply_filters('wutm_discord_notifications_capability', 'manage_woocommerce');
        add_submenu_page(
            'woocommerce',
            __('Discord 訂單通知', 'wu-discord-notifications'),
            __('Discord 訂單通知', 'wu-discord-notifications'),
            $capability,
            self::PAGE_SLUG,
    ...
    public function register_settings() {
        register_setting(self::OPTION_GROUP, 'wutm_discord_webhook_url', [
            'sanitize_callback' => function ($value) {
                return is_string($value) ? esc_url_raw(trim($value)) : '';
            },
        ]);
        register_setting(self::OPTION_GROUP, 'wutm_discord_order_statuses', [
            'sanitize_callback' => function ($value) {
                if (!is_array($value)) {
                    return [];
                }
        ...
        register_setting(self::OPTION_GROUP, 'wutm_discord_status_webhooks', [
            'sanitize_callback' => function ($value) {
                if (!is_array($value)) {
                    return [];
                }
        ...
        register_setting(self::OPTION_GROUP, 'wutm_discord_status_colors', [
            'sanitize_callback' => function ($value) {
                if (!is_array($value)) {
                    return [];
                }
        ...
        register_setting(self::OPTION_GROUP, 'wutm_discord_disable_image', [
            'sanitize_callback' => function ($value) {
                return !empty($value) ? 1 : 0;
            },
        ]);
        register_setting(self::OPTION_GROUP, 'wutm_discord_info_fields', [
            'sanitize_callback' => function ($value) {
        ...
        register_setting(self::OPTION_GROUP, 'wutm_discord_field_order', [
            'sanitize_callback' => static function ($value) {
                return self::sanitize_embed_field_order_value(is_array($value) ? $value : array());
            },
        ]);
        register_setting(self::OPTION_GROUP, 'wutm_discord_force_blocking', [
            'sanitize_callback' => function ($value) {
                return !empty($value) ? 1 : 0;
            },
        ]);
        register_setting(self::OPTION_GROUP, 'wutm_discord_notify_initiating_payment', [
            'sanitize_callback' => function ($value) {
                return !empty($value) ? 1 : 0;
            },
        ]);
        register_setting(self::OPTION_GROUP, 'wutm_discord_order_notes_customer_only', [
            'sanitize_callback' => function ($value) {
                return !empty($value) ? 1 : 0;
            },
        ]);
        ...
        add_settings_section(
            ...
            __('Discord Webhook 設定', 'wu-discord-notifications'),
            null,
            self::OPTION_GROUP
        );

        add_settings_field(
            'wutm_discord_webhook_url',
            __('Discord Webhook 網址', 'wu-discord-notifications'),
        ...
        add_settings_field(
            'wutm_discord_order_statuses',
            __('訂單狀態通知', 'wu-discord-notifications'),
        ...
        add_settings_field(
            'wutm_discord_disable_image',
            __('不顯示商品圖片', 'wu-discord-notifications'),
            function() {
                $disable_image = get_option('wutm_discord_disable_image');
                echo '<input type="hidden" name="wutm_discord_disable_image" value="0" />';
                echo '<input type="checkbox" name="wutm_discord_disable_image" value="1"' . checked(1, $disable_image, false) . '/>';
            },
            self::OPTION_GROUP,
            self::OPTION_GROUP . '_section'
        );
        ...
        add_settings_field(
            'wutm_discord_notify_initiating_payment',
            __('付款開始時發送通知', 'wu-discord-notifications'),
            function() {
                $enabled = (int) get_option('wutm_discord_notify_initiating_payment', 0);
                echo '<input type="hidden" name="wutm_discord_notify_initiating_payment" value="0" />';
                echo '<input type="checkbox" name="wutm_discord_notify_initiating_payment" value="1"' . checked(1, $enabled, false) . '/>';
                echo '<p class="description">' . esc_html__('顧客開始結帳並建立待付款訂單時發送通知；付款完成後仍會依訂單狀態發送正式通知。', 'wu-discord-notifications') . '</p>';
            },
            self::OPTION_GROUP,
            self::OPTION_GROUP . '_section'
        );
        ...
        add_settings_field(
            'wutm_discord_info_fields',
            __('通知欄位', 'wu-discord-notifications'),
        ...
        add_settings_field(
            'wutm_discord_field_order',
            __('通知欄位排序', 'wu-discord-notifications'),
        ...
        add_settings_field(
            'wutm_discord_order_notes_customer_only',
            __('僅顯示顧客備註', 'wu-discord-notifications'),
            function () {
                $customer_only = (int) get_option('wutm_discord_order_notes_customer_only', 0);
                echo '<input type="hidden" name="wutm_discord_order_notes_customer_only" value="0" />';
                echo '<input type="checkbox" name="wutm_discord_order_notes_customer_only" value="1"' . checked(1, $customer_only, false) . '/>';
                echo '<p class="description">' . esc_html__('啟用訂單備註欄位時，僅顯示顧客可見備註，不包含內部／管理員備註。', 'wu-discord-notifications') . '</p>';
            },
            self::OPTION_GROUP,
            self::OPTION_GROUP . '_section'
        );
        ...
        add_settings_field(
            'wutm_discord_force_blocking',
            __('除錯：強制同步傳送通知', 'wu-discord-notifications'),
            function () {
                $force = (int) get_option('wutm_discord_force_blocking', 0);
                echo '<input type="hidden" name="wutm_discord_force_blocking" value="0" />';
                echo '<label><input type="checkbox" name="wutm_discord_force_blocking" value="1" ' . checked(1, $force, false) . '> ' . esc_html__('僅在排除問題時啟用；同步傳送可能使結帳流程變慢。', 'wu-discord-notifications') . '</label>';
            },
            self::OPTION_GROUP,
            self::OPTION_GROUP . '_section'
        );
    ...
    public function info_fields_callback() {
        $info_fields = get_option('wutm_discord_info_fields', array());
        if (!is_array($info_fields)) {
            $info_fields = array();
        }
        $available = array(
            'status'        => __('訂單狀態', 'wu-discord-notifications'),
            'payment'       => __('付款資訊', 'wu-discord-notifications'),
            'product'       => __('商品明細', 'wu-discord-notifications'),
            'product_meta'  => __('商品選項', 'wu-discord-notifications'),
            'creation_date' => __('建立時間', 'wu-discord-notifications'),
            'billing'       => __('帳單資訊', 'wu-discord-notifications'),
            'customer_type' => __('顧客類型', 'wu-discord-notifications'),
            'shipping'      => __('運送方式', 'wu-discord-notifications'),
            'transaction_id' => __('交易編號', 'wu-discord-notifications'),
            'order_notes'   => __('訂單備註', 'wu-discord-notifications'),
        );
        echo '<p class="description">' . esc_html__('選擇要顯示在 Discord 通知中的欄位；未勾選任何欄位時會顯示全部。', 'wu-discord-notifications') . '</p>';
        foreach ($available as $key => $label) {
            $checked = in_array($key, $info_fields) ? 'checked' : '';
            echo '<p style="margin: 5px 0;">';
            echo '<label><input type="checkbox" name="wutm_discord_info_fields[]" value="' . esc_attr($key) . '" ' . esc_attr($checked) . '> ' . esc_html($label) . '</label>';
            echo '</p>';
        }
    }

    /**
     * Labels for embed field order UI (includes 訂單編號).
     *
     * @return array<string, string>
     */
    private function get_embed_field_labels(): array {
        return array(
            'order_id'       => __('訂單編號', 'wu-discord-notifications'),
            'status'         => __('訂單狀態', 'wu-discord-notifications'),
            'payment'        => __('付款資訊', 'wu-discord-notifications'),
            'product'        => __('商品明細', 'wu-discord-notifications'),
            'product_meta'   => __('商品選項', 'wu-discord-notifications'),
    ...
    public function field_order_callback() {
        $saved = get_option('wutm_discord_field_order', array());
        ...
        echo '<p class="description">' . esc_html__('拖曳列項可調整 Discord 通知欄位順序；上方未啟用的欄位傳送時仍會自動略過。', 'wu-discord-notifications') . '</p>';
        ...
        foreach ($order as $key) {
            ...
            echo '<input type="hidden" name="wutm_discord_field_order[]" value="' . esc_attr($key) . '" />';
    ...
    public function discord_webhook_url_callback() {
        $webhook_url = get_option('wutm_discord_webhook_url');
        echo '<input type="text" name="wutm_discord_webhook_url" value="' . esc_attr($webhook_url) . '" size="50" />';
    }

    public function discord_order_statuses_callback() {
        if (!function_exists('wc_get_order_statuses')) {
            echo '<p class="description">' . esc_html__('需先啟用 WooCommerce 才能顯示訂單狀態。', 'wu-discord-notifications') . '</p>';
            return;
        }
        $order_statuses = wc_get_order_statuses();
        if (empty($order_statuses)) {
            echo '<p class="description">' . esc_html__('目前沒有可用的訂單狀態。', 'wu-discord-notifications') . '</p>';
            return;
        }
        $selected_statuses = get_option('wutm_discord_order_statuses', []);
        if (!is_array($selected_statuses)) {
            $selected_statuses = [];
        }
        $status_webhooks = get_option('wutm_discord_status_webhooks', []);
        $status_colors = get_option('wutm_discord_status_colors', []);
        ...
        foreach ($order_statuses as $status => $label) {
            ...
            echo '<input type="checkbox" name="wutm_discord_order_statuses[]" value="' . esc_attr($status) . '" ' . esc_attr($checked) . '>';
            echo ' ' . esc_html($label);
            echo '</label>';
            echo '<input type="text" class="webhook-input" style="margin-right: 10px" name="wutm_discord_status_webhooks[' . esc_attr($status) . ']" value="' . esc_attr($webhook) . '" placeholder="Webhook 網址（選填）" size="50">';
            echo '<input type="text" name="wutm_discord_status_colors[' . esc_attr($status) . ']" value="' . esc_attr($color) . '" class="discord-embed-color-picker" />';
            echo '</p>';
        }
    }
    ...
    public function notification_settings_page() {
        ?>
        <div class="wrap">
            <h1><?php esc_html_e('Discord 訂單通知', 'wu-discord-notifications'); ?></h1>
    ...
    public function send_discord_notification($order_id) {
        ...
        if ($order->get_status() === 'pending') {
            if (get_option('wutm_discord_notify_initiating_payment')) {
    ...
    public function send_discord_notification_on_status_change($order_id, $old_status, $new_status, $order) {
        ...
        if ($new_status === 'pending' && get_option('wutm_discord_notify_initiating_payment')) {
            $this->maybe_send_initiating_notification($order_id, $order_obj);
            return;
        }

        $selected = get_option('wutm_discord_order_statuses', array());
    ...
    private function send_discord_notification_common($order_id, $type) {
        ...
        $webhook_url = !empty($status_webhooks[$order_status_key]) ? $status_webhooks[$order_status_key] : get_option('wutm_discord_webhook_url');
        ...
        $customer_only_notes = (bool) get_option('wutm_discord_order_notes_customer_only', false);
        $order_notes_text = '';
            ...
            switch ($field_key) {
                case 'order_id':
                    $embed_fields[] = array(
                        ...
                        'name'        => __('訂單編號', 'wu-discord-notifications'),
                ...
                case 'status':
                    ...
                    $embed_fields[] = array(
                        'internal_id' => 'status',
                        'name'        => __('訂單狀態', 'wu-discord-notifications'),
                        'value'       => $order_status,
                        'inline'      => false,
                    );
                    break;
                case 'payment':
                    ...
                    $embed_fields[] = array(
                        'internal_id' => 'payment',
                        'name'        => __('付款資訊', 'wu-discord-notifications'),
                ...
                case 'product':
                    ...
                    $embed_fields[] = array(
                        'internal_id' => 'product',
                        'name'        => __('商品明細', 'wu-discord-notifications'),
                        'value'       => $items_list ?: '-',
                        'inline'      => false,
                    );
                    break;
                case 'product_meta':
                    ...
                    $embed_fields[] = array(
                        ...
                        'name'        => __('商品選項', 'wu-discord-notifications'),
                ...
                case 'creation_date':
                    ...
                    $embed_fields[] = array(
                        ...
                        'name'        => __('建立時間', 'wu-discord-notifications'),
                ...
                case 'billing':
                    ...
                    $billing_value = "**姓名** » {$billing_first_name} {$billing_last_name}\n**電子郵件** » {$billing_email}";
                    if (!empty($billing_discord)) {
                        $billing_value .= "\n**Discord** » {$billing_discord}";
                    }
                    $embed_fields[] = array(
                        'internal_id' => 'billing',
                        'name'        => __('帳單資訊', 'wu-discord-notifications'),
                ...
                case 'customer_type':
                    ...
                    $embed_fields[] = array(
                        ...
                        'name'        => __('顧客類型', 'wu-discord-notifications'),
                ...
                case 'shipping':
                    ...
                    $embed_fields[] = array(
                        ...
                        'name'        => __('運送資訊', 'wu-discord-notifications'),
                ...
                case 'transaction_id':
                    ...
                    $embed_fields[] = array(
                        ...
                        'name'        => __('交易編號', 'wu-discord-notifications'),
                ...
                case 'order_notes':
                    ...
                    $embed_fields[] = array(
                        ...
                        'name'        => __('訂單備註', 'wu-discord-notifications'),
        ...
        if ($first_product_image && !get_option('wutm_discord_disable_image')) {
    ...
    private function get_embed_field_order_for_send(): array {
        ...
        return apply_filters('wutm_discord_field_order', $order);
    }
    ...
    private function get_embed_title($order, string $type): string {
        if ($type === 'initiating') {
            ...
            if (wcs_order_contains_subscription($order, 'renewal')) {
                ...
                $title = ($type === 'new') ? '🎉 新訂單！' : '🪄 訂單狀態更新！';
            }
        } else {
            $title = ($type === 'new') ? '🎉 新訂單！' : '🪄 訂單狀態更新！';
        }
        return apply_filters('wutm_discord_embed_title', $title, $order, $type);
    }
     ...
     * @return string Localized "新顧客", "回購顧客", or "未知".
     */
    private function get_customer_type_label_for_embed($order): string {
        if (!function_exists('wc_get_orders')) {
            return __('未知', 'wu-discord-notifications');
        }
        ...
        $statuses = apply_filters(
            'wutm_discord_returning_customer_order_statuses',
        ...
        $label = $is_returning
            ? __('回購顧客', 'wu-discord-notifications')
            : __('新顧客', 'wu-discord-notifications');
        return apply_filters('wutm_discord_customer_type_label', $label, $order, $is_returning);
    }
    ...
    private function get_order_notes_for_embed(int $order_id, $order, bool $customer_only = false): string {
        ...
        foreach ($notes as $note) {
            ...
            $type_label = ('customer' === $type) ? __('顧客', 'wu-discord-notifications') : __('內部', 'wu-discord-notifications');
    ...
    private function send_to_discord($webhook_url, $embed) {
        ...
        if (get_option('wutm_discord_force_blocking')) {
            $args['blocking'] = true;
            $args['timeout']  = 60;
        }
        ...
        if (is_wp_error($response)) {
            if (function_exists('wc_get_logger')) {
                wc_get_logger()->error(
                    ...
                    array('source' => 'wu-discord-notifications')
                );
            }
    ...
    public function plugin_action_links($links) {
        $settings_link = '<a href="' . esc_url(admin_url('admin.php?page=' . self::PAGE_SLUG)) . '">' . esc_html__('設定', 'wu-discord-notifications') . '</a>';
