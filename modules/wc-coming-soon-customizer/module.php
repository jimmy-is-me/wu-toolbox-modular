...
final class WUTM_WC_Coming_Soon_Customizer {
    ...
    public function register_frontend_hooks(): void { if ( is_admin() || wp_doing_ajax() || wp_doing_cron() || ! $this->is_customizer_active() || current_user_can( 'manage_woocommerce' ) ) return; add_action( 'wp_enqueue_scripts', [ $this, 'enqueue_frontend_style' ], 1 ); $this->override_home_product_shortcodes(); $s = $this->settings(); if ( $s['enabled'] && ( $s['title'] !== '' || $s['message'] !== '' ) ) add_filter( 'render_block', [ $this, 'replace_coming_soon_block' ], 20, 2 ); }
    ...
    public function replace_coming_soon_block( string $content, array $block ): string { if ( ( $block['blockName'] ?? '' ) !== 'woocommerce/coming-soon' ) return $content; $s = $this->settings(); $replace = []; if ( $s['title'] !== '' ) $replace = [ '大事即將發生' => $s['title'], 'Something big is coming' => $s['title'], 'Coming soon' => $s['title'] ]; if ( $s['message'] !== '' ) $replace += [ '有大事要發生了！ 我們正在籌備商店中，很快就會推出！' => $s['message'], '有大事要發生了！我們正在籌備商店中，很快就會推出！' => $s['message'], 'We are working on our store and will be back soon.' => $s['message'] ]; return $replace ? str_replace( array_keys( $replace ), array_values( $replace ), $content ) : $content; }
    public function enqueue_frontend_style(): void {
        $css = '.wp-block-woocommerce-coming-soon,.wp-block-woocommerce-coming-soon .wp-block-group,.wp-block-woocommerce-coming-soon h1,.wp-block-woocommerce-coming-soon h2,.wp-block-woocommerce-coming-soon p{font-family:inherit!important}.wp-block-woocommerce-coming-soon h1,.wp-block-woocommerce-coming-soon h2{font-style:inherit!important}';
