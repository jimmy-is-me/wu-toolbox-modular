...
final class WUTM_WC_Confetti {
    ...
    private function defaults(): array {
        return [
            ...
            'particles_per_side' => 60, 'second_wave_enabled' => 1, 'second_wave_count' => 24, 'delay_ms' => 120,
    ...
    public function sanitize( $input ): array {
        ...
        $out['particles_per_side'] = max( 1, min( 120, absint( $input['particles_per_side'] ?? 60 ) ) );
        $out['second_wave_count'] = max( 0, min( 60, absint( $input['second_wave_count'] ?? 24 ) ) );
    ...
    private function config( array $s ): array {
        ...
        return [ 'particles' => min( 80, (int) $s['particles_per_side'] ), 'secondEnabled' => ! empty( $s['second_wave_enabled'] ), 'secondCount' => min( 30, (int) $s['second_wave_count'] ), 'delay' => (int) $s['delay_ms'], 'speedMin' => (float) $s['speed_min'], 'speedMax' => (float) $s['speed_max'], 'gravityMin' => (float) $s['gravity_min'], 'gravityMax' => (float) $s['gravity_max'], 'durationMin' => (int) $s['duration_min'], 'durationMax' => (int) $s['duration_max'], 'drag' => (float) $s['drag'], 'colors' => $colors, 'zIndex' => (int) $s['z_index'] ];
    }
    ...
    private function render_script( array $config, bool $admin ): void { ?>
        ...
        <script>(function(){window.wutmWcConfetti=function(cfg){if(!cfg||!cfg.colors||!cfg.colors.length)return;if(window.matchMedia&&window.matchMedia('(prefers-reduced-motion: reduce)').matches)return;const lowPower=(navigator.connection&&navigator.connection.saveData)||((navigator.deviceMemory||8)<4);cfg.particles=Math.min(cfg.particles||0,lowPower?30:80);cfg.secondCount=Math.min(cfg.secondCount||0,lowPower?0:30);if(!cfg.particles)return;try{if(cfg.playOnce&&sessionStorage.getItem(cfg.key))return;if(cfg.playOnce)sessionStorage.setItem(cfg.key,'1')}catch(e){}const old=document.querySelector('.wutm-confetti-overlay');if(old)old.remove();const layer=document.createElement('div');layer.className='wutm-confetti-overlay';layer.style.zIndex=cfg.zIndex||999;document.body.appendChild(layer);const fragment=document.createDocumentFragment(),rnd=(a,b)=>Math.random()*(b-a)+a,parts=[];function add(count,second){for(let i=0;i<count;i++)['left','right'].forEach(side=>{const el=document.createElement('i'),circle=Math.random()<.1,w=circle?rnd(4,6):rnd(3,6),h=circle?w:rnd(7,12),angle=(side==='left'?rnd(-66,-43):rnd(-137,-114))*Math.PI/180,speed=rnd(cfg.speedMin,cfg.speedMax)*(second?rnd(.82,.92):1);el.className='wutm-confetti-piece';el.style.cssText+=';width:'+w+'px;height:'+h+'px;background:'+cfg.colors[Math.floor(Math.random()*cfg.colors.length)]+';border-radius:'+(circle?'50%':'1px');fragment.appendChild(el);parts.push({el:el,x:side==='left'?rnd(12,28):innerWidth-rnd(12,28),y:innerHeight-rnd(14,26),vx:Math.cos(angle)*speed,vy:Math.sin(angle)*speed,g:rnd(cfg.gravityMin,cfg.gravityMax),life:rnd(cfg.durationMin,cfg.durationMax)/1000,wait:rnd(0,.08),r:rnd(0,360),rs:rnd(-560,560),dead:false})});layer.appendChild(fragment)}add(cfg.particles,false);let start,second=false;const stop=()=>{if(layer.isConnected)layer.remove()};document.addEventListener('visibilitychange',()=>{if(document.hidden)stop()},{once:true});function frame(now){if(!layer.isConnected)return;start=start||now;const time=(now-start)/1000;if(cfg.secondEnabled&&!second&&time>=.28){second=true;add(cfg.secondCount,true)}let alive=0;parts.forEach(p=>{if(p.dead)return;const t=time-p.wait;if(t<0){alive++;return}if(t>p.life){p.dead=true;p.el.remove();return}alive++;const x=p.x+(p.vx/cfg.drag)*(1-Math.exp(-cfg.drag*t)),y=p.y+p.vy*t+.5*p.g*t*t,progress=t/p.life,opacity=progress<.03?progress/.03:(progress>.84?(1-progress)/.16:1);p.el.style.opacity=Math.max(0,opacity);p.el.style.transform='translate3d('+x+'px,'+y+'px,0) rotate('+(p.r+p.rs*t)+'deg)'}) ;if(alive)requestAnimationFrame(frame);else stop()}requestAnimationFrame(frame)};<?php if ( ! $admin ) : ?>const cfg=<?php echo wp_json_encode( $config ); ?>;const run=()=>setTimeout(()=>window.wutmWcConfetti(cfg),cfg.delay||0);document.readyState==='complete'?run():addEventListener('load',run,{once:true});<?php endif; ?>})();</script>
    <?php }
}
