...
/** 提供免運門檻使用的預估新會員折扣；不新增 fee，也不改變購物車。 */
...
function wutm_nmd_notice():void{
    ...
    echo '<div class="wutm-nmd-notice" style="margin:15px 0;padding:12px 16px;border:1px solid #d6e7dc;border-radius:7px;background:#f4faf6;color:#185c37">🎉 <strong>新會員優惠：</strong>首次下單滿 '.wp_kses_post(wc_price($o['minimum'])).'，自動折抵 '.wp_kses_post(wc_price($o['discount'])).'。<strong>免運門檻以折扣後小計判定；例如商品 NT$2,000、優惠折 NT$100，小計為 NT$1,900，尚未達 NT$2,000 免運門檻。</strong>'.(!is_user_logged_in()?'登入會員後即可套用。':'').'</div>';
}
