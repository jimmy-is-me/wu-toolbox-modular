...
class Skip_Rows {
	...
	public function skip_current(): void {
		if ( '' === $this->table || 0 === $this->row_id ) {
			_doing_it_wrong( __METHOD__, esc_html__( 'You must set a table and DB id before skipping a row.', 'wutm-go-live-update-urls' ), '6.5.0' );
		}
	...
	public function log_error( string $class_name ): void {
		...
		\error_log( \vsprintf( '網址更新 skipped row `%s` in the table `%s` because it contains an unavailable PHP class named `%s`.', [
			$this->row_id,
			$this->table,
			$class_name,
		] ) );
