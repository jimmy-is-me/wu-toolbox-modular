...
function wutm_fs_add_threshold_field(array $fields): array { $fields['wutm_free_shipping_threshold']=['title'=>'免運門檻 (NT$)','type'=>'price','description'=>'商品金額扣除折價券與新會員優惠後的小計（不含運費及稅）達此金額時，此運送方式自動免運。留空或 0 表示停用。','default'=>'','desc_tip'=>true];return $fields; }
...
/**
 * 免運門檻採用折扣後商品小計：一般折價券與新會員優惠都會扣除。
 * 新會員優惠為 fee，運費通常會在 fee 建立前計算，因此同步預估其金額。
 */
function wutm_fs_cart_subtotal(): float {
    ...
    $new_member_discount = 0.0;
    foreach ((array) WC()->cart->get_fees() as $fee) {
        if (strpos((string) $fee->name, '新會員優惠') === 0 && (float) $fee->total < 0) {
            $new_member_discount = abs((float) $fee->total);
            break;
        }
    }
    if ($new_member_discount <= 0 && function_exists('wutm_nmd_cart_discount_amount')) {
        $new_member_discount = wutm_nmd_cart_discount_amount(WC()->cart);
    }
    return max(0, $subtotal - $new_member_discount);
}
