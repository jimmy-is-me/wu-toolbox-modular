...
function wutm_smb_shortcode($atts): string {
    ...
    $icon_color = sanitize_hex_color($atts['icon_color']) ?: $button_color;
    ...
    ob_start();
    ?>
    <div id="<?php echo esc_attr($uid); ?>" class="wutm-smb" style="--wutm-smb-width:<?php echo esc_attr(wutm_smb_css_size($o['max_width'])); ?>;--wutm-smb-accent:<?php echo esc_attr($accent); ?>;--wutm-smb-button-color:<?php echo esc_attr($button_color); ?>;--wutm-smb-icon-color:<?php echo esc_attr($icon_color); ?>">
        <?php if ($mode === 'popup'): ?>
            <button type="button" class="wutm-smb-trigger" aria-haspopup="dialog"><span class="wutm-smb-icon" aria-hidden="true">✉</span><span class="wutm-smb-trigger-text"><?php echo esc_html($button_text); ?></span></button>
        ...
        <div class="wutm-smb-card">
            ...
            <form id="<?php echo esc_attr($form_id); ?>" class="wutm-smb-form">
                <fieldset><legend>聯絡方式</legend><label><input type="radio" name="method" value="email" checked> Email</label><label><input type="radio" name="method" value="phone"> 電話</label></fieldset>
    ...
    <style>
        #<?php echo esc_attr($uid); ?>{width:100%;max-width:var(--wutm-smb-width);margin:24px auto;box-sizing:border-box;font:inherit;color:inherit}#<?php echo esc_attr($uid); ?> *{box-sizing:border-box}#<?php echo esc_attr($uid); ?> .wutm-smb-card{padding:30px;border:1px solid #e1e4e8;border-radius:14px;background:#fff;box-shadow:0 8px 28px rgba(20,30,50,.06)}#<?php echo esc_attr($uid); ?> h2{margin:0 0 8px;font-size:26px;line-height:1.35}#<?php echo esc_attr($uid); ?> .wutm-smb-intro{margin:0 0 24px;color:#5f6875}#<?php echo esc_attr($uid); ?> fieldset{display:flex;gap:22px;margin:0 0 20px;padding:0;border:0}#<?php echo esc_attr($uid); ?> legend{margin-bottom:9px;font-weight:700}#<?php echo esc_attr($uid); ?> fieldset label{display:inline-flex;gap:7px;align-items:center}#<?php echo esc_attr($uid); ?> .wutm-smb-field{display:block;margin-bottom:18px;font-weight:600}#<?php echo esc_attr($uid); ?> .wutm-smb-field>span{display:block;margin-bottom:8px}#<?php echo esc_attr($uid); ?> b{color:#d63638}#<?php echo esc_attr($uid); ?> input[type=email],#<?php echo esc_attr($uid); ?> input[type=tel],#<?php echo esc_attr($uid); ?> textarea{display:block;width:100%;padding:12px 14px;border:1px solid #cfd5dc;border-radius:8px;background:#fff;color:#1d2327;font:inherit;line-height:1.5}#<?php echo esc_attr($uid); ?> input:focus,#<?php echo esc_attr($uid); ?> textarea:focus{border-color:var(--wutm-smb-accent);outline:2px solid color-mix(in srgb,var(--wutm-smb-accent) 20%,transparent);outline-offset:1px}#<?php echo esc_attr($uid); ?> .wutm-smb-submit,#<?php echo esc_attr($uid); ?> .wutm-smb-trigger{padding:12px 24px;border:0;border-radius:8px;background:var(--wutm-smb-accent);color:var(--wutm-smb-button-color);font:inherit;font-weight:700;cursor:pointer}#<?php echo esc_attr($uid); ?> .wutm-smb-icon{color:var(--wutm-smb-icon-color)}#<?php echo esc_attr($uid); ?> .wutm-smb-submit:disabled{opacity:.6;cursor:wait}#<?php echo esc_attr($uid); ?> .wutm-smb-status{margin-top:14px;font-weight:600}#<?php echo esc_attr($uid); ?> .wutm-smb-status.is-success{color:#008a20}#<?php echo esc_attr($uid); ?> .wutm-smb-status.is-error{color:#b32d2e}#<?php echo esc_attr($uid); ?> .wutm-smb-hp{position:absolute!important;left:-10000px!important;width:1px!important;height:1px!important;overflow:hidden!important}#<?php echo esc_attr($uid); ?> .wutm-smb-trigger{display:inline-flex;gap:9px;align-items:center}<?php if ($display === 'icon'): ?>#<?php echo esc_attr($uid); ?> .wutm-smb-trigger-text{position:absolute;width:1px;height:1px;overflow:hidden;clip:rect(0 0 0 0)}<?php elseif ($display === 'text'): ?>#<?php echo esc_attr($uid); ?> .wutm-smb-icon{display:none}<?php endif; ?>#<?php echo esc_attr($uid); ?> .wutm-smb-modal{position:fixed;z-index:999999;inset:0;padding:24px;overflow:auto}#<?php echo esc_attr($uid); ?> .wutm-smb-backdrop{position:fixed;inset:0;background:rgba(0,0,0,.56)}#<?php echo esc_attr($uid); ?> .wutm-smb-dialog{position:relative;z-index:1;width:min(100%,var(--wutm-smb-width));margin:5vh auto}#<?php echo esc_attr($uid); ?> .wutm-smb-close{position:absolute;z-index:2;top:10px;right:12px;width:40px;height:40px;padding:0;border:0;background:transparent;color:#555;font-size:30px;line-height:1;cursor:pointer}@media(max-width:600px){#<?php echo esc_attr($uid); ?>{padding:0 12px}#<?php echo esc_attr($uid); ?> .wutm-smb-card{padding:22px 18px}#<?php echo esc_attr($uid); ?> .wutm-smb-modal{padding:10px}#<?php echo esc_attr($uid); ?> .wutm-smb-dialog{margin:2vh auto}}
    </style>
    <script>
    (function(){
